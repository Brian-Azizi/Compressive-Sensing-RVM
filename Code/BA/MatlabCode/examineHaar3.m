...
BASIS3 = [kron(sPhi3*sPhi2*sPhi,kron(cPhi3*cPhi2*cPhi,rPhi3*rPhi2*rPhi)); ...
...
% 
BASIS3 = BASIS3';
outFile = strcat('../testFiles/basis_3_',num2str(height),'-',num2str(width),'-',num2str(frames),'.txt');
save(outFile,'BASIS3','-ascii');

% A = int32(normFactor^3*BASIS3);
% B = int32(normFactor^3*cpp_basis);
% isequal(A,B)
% isequal(BASIS3,cpp_basis)
% sum(sum(abs(BASIS3-cpp_basis)))/height/frames/width
% 
% CC = abs(BASIS3-cpp_basis)>0.0001;
% fprintf('Error is %d out of %d\n',sum(sum(CC)),(height*width*frames)^2);
% 
% LLL_true = BASIS3(:,1:dSize/8);
