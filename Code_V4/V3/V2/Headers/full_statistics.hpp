#ifndef GUARD_FULL_STATISTICS_HPP
#define GUARD_FULL_STATISTICS_HPP

#include "utilities.hpp"

#include <iomanip>

void full_statistics(double **BASIS, double **PHI, int included_basis, double *targets, int *indeces_used,
		     bool *ins_model, double *alphas, double beta, double *mean, double **covariance,
		     double *S_in, double *Q_in, double *S_out, double *Q_out, double *relevance_factors,
		     int N, int M, double **BASIS_PHI, double **BETA_BASIS_PHI, double *BASIS_Targets, double *gamma)
{    
    uint64 focusTime1S = GetTimeMs64();	
    double **temp_cova = new double*[included_basis];
    for (int i=0;i<included_basis;i++) temp_cova[i] = new double[included_basis];
    
    double **temp_temp_cova = new double*[included_basis];
    for (int i=0;i<included_basis;i++) temp_temp_cova[i] = new double[included_basis];		
    
    double **temp_PHI = new double*[N];
    for (int i=0;i<N;i++) temp_PHI[i] = new double[included_basis];
    
    double **temp_PHI_transpose = new double*[included_basis];
    for (int i=0;i<included_basis;i++) temp_PHI_transpose[i] = new double[N];		
    ...
    for (int i=0;i<included_basis;i++) chol[i] = new double[included_basis];
    
    double **inv_chol = new double*[included_basis];
    for (int i=0;i<included_basis;i++) inv_chol[i] = new double[included_basis];		
    ...
    for (int i=0;i<included_basis;i++) tran_inv_chol[i] = new double[included_basis];
    ...
    for (int rows=0;rows<included_basis;rows++) temp_mean[rows] = beta*temp_mean[rows];
        
    double *y = new double[N];
    multiply2D1D(temp_PHI, temp_mean, y, N, included_basis);
    double *residual = new double[N];
    for (int rows=0;rows<N;rows++) residual[rows] = targets[rows] - y[rows];
    
    // calculating sparsity and quality factors
    double *sum_inv_chol = new double[included_basis];
    for (int rows=0; rows<included_basis;rows++) sum_inv_chol[rows] = 0;    
    ...
    for (int i=0;i<M;i++) BETA_chol[i] = new double[included_basis];
