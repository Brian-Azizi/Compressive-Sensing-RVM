...
This is the \emph{separability} of the DCT.
...
In Figure \ref{fig:2D-DCT}, we show the 2-D basis functions that are used by the DCT to decompose a signal of size $4\times 4$.
...
For a deeper discussion on the properties of the DCT, see \cite{khayam2003}.
...
We will discuss how a wavelet basis can be constructed at multiple resolution levels and how the DWT of a signal can be computed in practice.
The aim of this section is to provide some intuition for the general properties of wavelets, particularly for the \emph{Haar wavelets}.
We will refrain from going to deeply into the mathematical details and instead direct the reader to \cite{mallat1999,daubechies1992}.
...
For the Haar wavelets, the scaling function is given by
\begin{equation}
...
\footnote{
  This construction procedure is the so-called \emph{non-standard} construction.
...
Looking at the detail coefficients, one can observe that the output of the high-pass filters of the Haar wavelets correlate with edges in the image.
...
The arrangement of the output channels of the various filters is shown in Figure \ref{fig:3d_channels}.

For the Haar wavelets, the high-pass filters act as edge detectors, similar to the 2-D case. 
...
In panel (c), we see the spatial edge detectors (the $LLH$, $LHL$ and $LHH$ channels) that are comparable to those of the 2-D transform (Figure \ref{fig:dwt2}).
...
The basis matrix, in this case, is simply the inverse transform matrix.
...
Let $\bm a_j^T$ be the $j$th row of $\bm A\in\mathbb{R}^{r\times c}$.
...
$}
\end{equation*}
which has dimensions $(hwf)\times(hwf)$.
